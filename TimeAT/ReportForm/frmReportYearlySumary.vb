...
Public Class frmReportYearlySumary
    ...
    Private Sub btnReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReport.Click
        ...
        exsheet.Cells(1, 1).value = "โรงทำงานเซนต์ปอลคอนแวนต์"
        exsheet.Cells(2, 1).value = "สรุปยอดประจำปี " & txtYear.Text.Trim

        exsheet.Cells(5, 1).value = "NO."
        exsheet.Cells(5, 2).value = "Code"
        exsheet.Cells(5, 3).value = "ชื่อ - สกุล"
        exsheet.Range("C5", "D5").Merge(1)

        exsheet.Cells(4, 5).value = "พ.ค."
        exsheet.Range(GenColumnExcel(5, 4), GenColumnExcel(8, 4)).Merge(1)
        exsheet.Cells(5, 5).value = "มา"
        exsheet.Cells(5, 6).value = "สาย"
        exsheet.Cells(5, 7).value = "ลา"
        exsheet.Cells(5, 8).value = "ขาด"

        exsheet.Cells(4, 9).value = "มิ.ย."
        exsheet.Range(GenColumnExcel(9, 4), GenColumnExcel(12, 4)).Merge(1)
        exsheet.Cells(5, 9).value = "มา"
        exsheet.Cells(5, 10).value = "สาย"
        exsheet.Cells(5, 11).value = "ลา"
        exsheet.Cells(5, 12).value = "ขาด"

        exsheet.Cells(4, 13).value = "ก.ค."
        exsheet.Range(GenColumnExcel(13, 4), GenColumnExcel(16, 4)).Merge(1)
        exsheet.Cells(5, 13).value = "มา"
        exsheet.Cells(5, 14).value = "สาย"
        exsheet.Cells(5, 15).value = "ลา"
        exsheet.Cells(5, 16).value = "ขาด"

        exsheet.Cells(4, 17).value = "ส.ค."
        exsheet.Range(GenColumnExcel(17, 4), GenColumnExcel(20, 4)).Merge(1)
        exsheet.Cells(5, 17).value = "มา"
        exsheet.Cells(5, 18).value = "สาย"
        exsheet.Cells(5, 19).value = "ลา"
        exsheet.Cells(5, 20).value = "ขาด"

        exsheet.Cells(4, 21).value = "ก.ย."
        exsheet.Range(GenColumnExcel(21, 4), GenColumnExcel(24, 4)).Merge(1)
        exsheet.Cells(5, 21).value = "มา"
        exsheet.Cells(5, 22).value = "สาย"
        exsheet.Cells(5, 23).value = "ลา"
        exsheet.Cells(5, 24).value = "ขาด"

        exsheet.Cells(4, 25).value = "ต.ค."
        exsheet.Range(GenColumnExcel(25, 4), GenColumnExcel(28, 4)).Merge(1)
        exsheet.Cells(5, 25).value = "มา"
        exsheet.Cells(5, 26).value = "สาย"
        exsheet.Cells(5, 27).value = "ลา"
        exsheet.Cells(5, 28).value = "ขาด"

        exsheet.Cells(4, 29).value = "พ.ย."
        exsheet.Range(GenColumnExcel(29, 4), GenColumnExcel(32, 4)).Merge(1)
        exsheet.Cells(5, 29).value = "มา"
        exsheet.Cells(5, 30).value = "สาย"
        exsheet.Cells(5, 31).value = "ลา"
        exsheet.Cells(5, 32).value = "ขาด"

        exsheet.Cells(4, 33).value = "ธ.ค."
        exsheet.Range(GenColumnExcel(33, 4), GenColumnExcel(36, 4)).Merge(1)
        exsheet.Cells(5, 33).value = "มา"
        exsheet.Cells(5, 34).value = "สาย"
        exsheet.Cells(5, 35).value = "ลา"
        exsheet.Cells(5, 36).value = "ขาด"

        exsheet.Cells(4, 37).value = "ม.ค."
        exsheet.Range(GenColumnExcel(37, 4), GenColumnExcel(40, 4)).Merge(1)
        exsheet.Cells(5, 37).value = "มา"
        exsheet.Cells(5, 38).value = "สาย"
        exsheet.Cells(5, 39).value = "ลา"
        exsheet.Cells(5, 40).value = "ขาด"

        exsheet.Cells(4, 41).value = "ก.พ."
        exsheet.Range(GenColumnExcel(41, 4), GenColumnExcel(44, 4)).Merge(1)
        exsheet.Cells(5, 41).value = "มา"
        exsheet.Cells(5, 42).value = "สาย"
        exsheet.Cells(5, 43).value = "ลา"
        exsheet.Cells(5, 44).value = "ขาด"

        exsheet.Cells(4, 45).value = "มี.ค."
        exsheet.Range(GenColumnExcel(45, 4), GenColumnExcel(48, 4)).Merge(1)
        exsheet.Cells(5, 45).value = "มา"
        exsheet.Cells(5, 46).value = "สาย"
        exsheet.Cells(5, 47).value = "ลา"
        exsheet.Cells(5, 48).value = "ขาด"

        exsheet.Cells(4, 49).value = "เม.ย."
        exsheet.Range(GenColumnExcel(49, 4), GenColumnExcel(52, 4)).Merge(1)
        exsheet.Cells(5, 49).value = "มา"
        exsheet.Cells(5, 50).value = "สาย"
        exsheet.Cells(5, 51).value = "ลา"
        exsheet.Cells(5, 52).value = "ขาด"
